...
def _diff_to_tpfp(diff, threshold):
    ...
    hit = np.zeros(diff.shape[1], bool)
    tp = np.zeros(diff.shape[0], bool)
    fp = np.zeros(diff.shape[0], bool)
    ...
    def _update_with_label(self, lines_pred, lines_gt, scores, labels_pred, labels_gt):
        for l, l_name in zip(self.labels_idx, self.labels):
            ...
            if np.any(gt_mask):
                ...
                tp = np.zeros(nbr_det, dtype=bool)
                fp = np.ones(nbr_det, dtype=bool)
...
def evalulate_sap(result_list, annotations_dict, thresholds, labels):
    ...
    for res in result_list:
        ...
        lines_pred[:,0] *= 64/float(res['width'])
        lines_pred[:,1] *= 64/float(res['height'])
        lines_pred[:,2] *= 64/float(res['width'])
        lines_pred[:,3] *= 64/float(res['height'])

        junctions_gt = np.array(gt['junctions'].detach().cpu(), dtype=np.float32)
        lines_gt = []
        ...
        lines_gt[:,0]  *= 64/float(gt['width'])
        lines_gt[:,1]  *= 64/float(gt['height'])
        lines_gt[:,2]  *= 64/float(gt['width'])
        lines_gt[:,3]  *= 64/float(gt['height'])

        labels_gt = np.array(gt['edges_semantic'].detach().cpu(), dtype=int)
...
def evalulate_jap(result_list, annotations_dict, thresholds, labels):
    ...
    for res in result_list:
        ...
        juncs_pred[:,0] *= 64/float(res['width'])
        juncs_pred[:,1] *= 64/float(res['height'])

        juncs_gt = np.array(gt['junctions'].detach().cpu(),dtype=np.float32)
        juncs_gt[:,0] *= 64/float(res['width'])
        juncs_gt[:,1] *= 64/float(res['height'])

        if 'junctions_semantic' in gt:
            labels_gt = np.array(gt['junctions_semantic'].detach().cpu(),dtype=int)
        else:
            occluded = np.array(gt['junc_occluded'], dtype=bool)
            labels_gt = np.zeros(occluded.shape, dtype=int)
        ...
        for k, eval_type_meters in eval_meters.items():
            ...
            if k == 'label':
                labels = np.array(res['juncs_label'], dtype=int)
                ...
                line_labels = np.array(res['lines_label'], dtype=int)
                line2junc_idx = np.array(res['line2junc_idx'], dtype=int)
