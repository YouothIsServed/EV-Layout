...
class WireframeDetector(nn.Module):
    ...
    def forward_test(self, images, annotations = None, output_features = False):
        ...
        # outpath = '/home/sduu2/userspace-18T-2/gxc/project/process_all_need_data/check_featuremap/orig_feature/'
        # np.save(outpath + 'md.npy',md_pred.detach().cpu().numpy())
        # np.save(outpath + 'dis.npy', dis_pred.detach().cpu().numpy())
        # np.save(outpath + 'res.npy', res_pred.detach().cpu().numpy())
        # np.save(outpath + 'jlabel.npy', jlabel_prob.detach().cpu().numpy())
        # np.save(outpath + 'jloc.npy', jloc_pred.detach().cpu().numpy())
        # np.save(outpath + 'joff.npy', joff_pred.detach().cpu().numpy())
    ...
    def forward_train(self, images, annotations = None):
        new_dataset_path = '/home/sduu2/userspace-18T-1/gxc/dataset/Dataset_for_train/train'
        img_counts = glob.glob(os.path.join(new_dataset_path + '/images/', '*.npy'))
        images1 = images.squeeze(0)[:, :, :]
        # np.save(new_dataset_path + '/images/' + str('%06d' % len(img_counts)) + '.npy', images1.detach().cpu().numpy())
        ...
        gt1_counts = glob.glob(os.path.join(new_dataset_path + '/gt1/', '*.npy'))
        gt2_counts = glob.glob(os.path.join(new_dataset_path + '/gt2/', '*.npy'))

        # np.save(new_dataset_path + '/gt1/' + str('%06d' % len(gt1_counts)) + '.npy', targets)
        # np.save(new_dataset_path + '/gt2/' + str('%06d' % len(gt2_counts)) + '.npy', metas)
        ...
        loi_features = self.fc1(features)
        # print('loi_features{}'.format(loi_features.shape))
        md_out, dis_out, res_out, jlabel_out, joff_out = self._get_output_dist(outputs[0])

        md_pred = md_out.sigmoid()
        # print('md.{}'.format(md_pred.shape))
        dis_pred = dis_out.sigmoid()
        # print('dis.{}'.format(dis_pred.shape))
        res_pred = res_out.sigmoid()
        # print('res.{}'.format(res_pred.shape))
        jlabel_prob = jlabel_out.softmax(1)
        jlabel = jlabel_prob.argmax(1)
        # print('jlabel.{}'.format(jlabel.shape))

        jloc_pred = 1-jlabel_prob[:,0,None]
        # print('jloc.{}'.format(jloc_pred.shape))
        joff_pred= joff_out.sigmoid() - 0.5
        # print('joff.{}'.format(joff_pred.shape))
