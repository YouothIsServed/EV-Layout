...
def train(cfg):
    ...
    test_dataset = build_test_dataset(cfg, validation=True)
    ...
    if resume==True:
        ...
        checkpoint = torch.load('./outputs/checkpoint_1map_main/stage_4/latest_swin_hawp_61.pth')
        checkpoint_model = checkpoint['model']
        model_state_dict = model.state_dict()
        new_state_dict = {}
        for key in model_state_dict.keys():
            if key in checkpoint_model:
                new_state_dict[key] = checkpoint_model[key]
            else:
    ...
    for epoch in range(epochs):
        ...
        for images, annotations in tqdm(train_dataset):
            ...
            map_orig2 = torch.cat([annotations[item]['map_orig2'].unsqueeze(0) for item in range(images.shape[0])], dim=0)
            data['map_orig2'] = to_device(map_orig2, device)
            map_shift2 = torch.cat([annotations[item]['map_shift2'].unsqueeze(0) for item in range(images.shape[0])],
                                  dim=0)
            data['map_shift2'] = to_device(map_shift2, device)
                ...
                with torch.no_grad():
                    for name, dataset in test_dataset:
                        ...
                        for i, (images, annotations) in enumerate(tqdm(dataset)):
                            ...
                            map_orig2 = ann['map_orig2'].unsqueeze(0)
                            map_shift2 = ann['map_shift2'].unsqueeze(0)
                            img['map_orig'] = to_device(map_orig, device)
                            img['map_shift'] = to_device(map_shift, device)
                            img['map_orig2'] = to_device(map_orig2, device)
                            img['map_shift2'] = to_device(map_shift2, device)
